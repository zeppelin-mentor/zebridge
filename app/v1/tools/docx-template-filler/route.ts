...
import { docxTemplateFiller } from '@/lib/tools/document-tools'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: execution } = await supabase
      .from('executions')
      .insert({ user_id: authContext.userId, api_key_id: authContext.apiKeyId, tool_slug: 'docx-template-filler', input_params: { templateUrl: input.templateUrl, variableCount: Object.keys(input.variables).length }, status: 'processing' })
      .select('id').single()

    if (!execution) throw new Error('Failed to create execution record')
    executionId = execution.id

    const result = await docxTemplateFiller(input, authContext.userId, execution.id)
    ...
    console.error('Error in docx-template-filler API:', error)
