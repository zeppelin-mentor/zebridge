...
import { sendWebhook } from '@/lib/tools/communication-tools'
import { z } from 'zod'
...
const Schema = z.object({
  ...
  platform: z.enum(['slack', 'discord', 'generic']).optional(),
})
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: execution } = await supabase
      .from('executions')
      .insert({ user_id: authContext.userId, api_key_id: authContext.apiKeyId, tool_slug: 'send-webhook', input_params: { platform: input.platform || 'generic' }, status: 'processing' })
      .select('id').single()

    if (!execution) throw new Error('Failed to create execution record')
    executionId = execution.id

    const result = await sendWebhook(input, authContext.userId, execution.id)
    ...
    console.error('Error in send-webhook API:', error)
