...
import { docxToMarkdown } from '@/lib/tools/document-tools'
import { z } from 'zod'

const Schema = z.object({
  docxUrl: z.string().url(),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: execution } = await supabase
      .from('executions')
      .insert({ user_id: authContext.userId, api_key_id: authContext.apiKeyId, tool_slug: 'docx-to-markdown', input_params: input, status: 'processing' })
      .select('id').single()

    if (!execution) throw new Error('Failed to create execution record')
    executionId = execution.id

    const result = await docxToMarkdown(input, authContext.userId, execution.id)
    ...
    console.error('Error in docx-to-markdown API:', error)
