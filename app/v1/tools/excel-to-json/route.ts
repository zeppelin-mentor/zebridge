...
import { excelToJson } from '@/lib/tools/document-tools'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: execution } = await supabase
      .from('executions')
      .insert({ user_id: authContext.userId, api_key_id: authContext.apiKeyId, tool_slug: 'excel-to-json', input_params: input, status: 'processing' })
      .select('id').single()

    if (!execution) throw new Error('Failed to create execution record')
    executionId = execution.id

    const result = await excelToJson(input, authContext.userId, execution.id)
    ...
    console.error('Error in excel-to-json API:', error)
