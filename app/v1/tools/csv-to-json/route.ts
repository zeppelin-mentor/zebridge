...
import { csvToJson } from '@/lib/tools/document-tools'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: execution } = await supabase
      .from('executions')
      .insert({ user_id: authContext.userId, api_key_id: authContext.apiKeyId, tool_slug: 'csv-to-json', input_params: { delimiter: input.delimiter }, status: 'processing' })
      .select('id').single()

    if (!execution) throw new Error('Failed to create execution record')
    executionId = execution.id

    const result = await csvToJson(input, authContext.userId, execution.id)
    const duration = Date.now() - startTime

    await supabase.from('executions').update({ status: 'success', output_metadata: { outputUrl: result.outputUrl, filename: result.filename, rowCount: result.rowCount }, duration_ms: duration, completed_at: new Date().toISOString() }).eq('id', execution.id)
    ...
    console.error('Error in csv-to-json API:', error)
